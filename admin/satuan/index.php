...
$query = mysqli_query(
    $koneksi,
    "SELECT * FROM satuan
    WHERE nama_satuan LIKE '%$cari%'
    ORDER BY nama_satuan ASC"
);
?>

<!-- popup berhasil hapus -->
<?php if (isset($_SESSION['success_satuan'])) : ?>
<div id="alertSuccess" class="alert alert-success">
    <?= $_SESSION['success_satuan']; ?>
</div>
<?php unset($_SESSION['success_satuan']); ?>
<?php endif; ?>
...
<body class="bg-light">
    ...
    <div class="container py-5">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="fw-bold mb-1">📏 Kelola Satuan</h2>
                <p class="text-muted mb-0">Daftar satuan produk minimarket</p>
            </div>
        ...
        <form method="GET" class="mb-4" id="formCari">
            <div class="input-group shadow-sm">
                ...
                <input type="text" id="inputCari" name="cari" class="form-control border-0"
                    placeholder="Cari nama satuan..." value="<?= htmlspecialchars($cari); ?>">
            </div>
        </form>
            ...
            <div class="card-header bg-white border-0 py-3">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <span class="text-muted small">
                        Total: <?= mysqli_num_rows($query); ?> satuan
                    </span>
                </div>
            </div>
            <div class="card-body p-0">
                <div class="table-scroll">
                    <table class="table table-hover align-middle text-center mb-0">
                        ...
                        <tbody>
                            <?php $no = 1; while ($satuan = mysqli_fetch_assoc($query)) : ?>
                            <tr>
                                <td><?= $no++; ?></td>
                                <td class="fw-semibold">
                                    <span class="rounded-pill px-3 py-2">
                                        <?= $satuan['nama_satuan']; ?>
                                    </span>
                                </td>
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="edit.php?id=<?= $satuan['id_satuan']; ?>"
                                            class="btn btn-sm btn-outline-warning rounded-3 px-3">
                                            ✏️
                                        </a>
                                        <a href="../../proses/admin/hapus/proses_hapus_satuan.php?id=<?= $satuan['id_satuan']; ?>"
